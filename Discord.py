...
async def resume_song(ctx):
    ...
    try:
        if pause == True:
            voice_client.resume()
            await ctx.send(embed=discord.Embed(description="Song resumed [▶]", color=discord.Color.from_rgb(0, 255, 255)))
        else:
            if playing == True:
...
async def clear_song_queue(ctx):
    ...
    if len(queue) > 0:
        ...
        queue.clear()
        embed = discord.Embed(color=discord.Color.from_rgb(0, 255, 255))
        embed.set_author(name="Queue has been cleared [✅]", icon_url=url_author_music)
        await ctx.send(embed=embed)
    else:
...
print(txt_from_file[start_token:end_token])
