...
color = discord.Color.from_rgb(87, 1, 254)  # 87, 1, 254 | 65, 95, 255 |
...
dialogue_list = []
# SQL
conn = ms.connect(user="root", host="localhost",
                  password=os.getenv('sql_pass'), database="discord")
cursor = conn.cursor()
...
async def on_ready():
    ...
    for i in range(0, 1000):
        q = site.find('<p class="has-background" style="background-color:#dedfe0">',
                      stop) + len('<p class="has-background style="background-color:#dedfe0">')
        w = site.find("</p>", stop)
        stop = w + len("</p>")
        dialogues = ""
        if not site[q:w]:
        ...
        if reaction.emoji == "🖱":
            ...
            try:
                sub = reddit.subreddit(
                    default_topic[str(reaction.message.guild.id)]).random()
                embed = discord.Embed(
                    description="**Caption:\n**{}".format(sub.title), color=color)
                embed.set_author(name="Post by: {}".format(
                    sub.author), icon_url=url_reddit_author)
                ...
                if members_in_vc.count(str(user)) > 0:
                    try:
                        ...
                        if server_index[str(reaction.message.guild.id)] > 10:
                            index = server_index[str(
                                reaction.message.guild.id)] - 10
                ...
                if members_in_vc.count(str(user)) > 0:
                    ...
                    try:
                        URL_queue = youtube_download(
                            reaction.message, server_queue[server_index[str(reaction.message.guild.id)]][1])

                        if playing != True:
                            embed = discord.Embed(description="**Song: **{a}\n**Queue Index: **{b}".format(a=server_queue[server_index[str(
                                reaction.message.guild.id)]][0], b=server_index[str(reaction.message.guild.id)]).replace(" - YouTube", " "), color=color)
                            embed.set_author(
                                name="Now playing", icon_url=url_author_music)
                            embed.set_thumbnail(url=pytube.YouTube(
                                url=server_queue[server_index[str(reaction.message.guild.id)]][1]).thumbnail_url)
                            embed.add_field(name="Uploader", value=pytube.YouTube(
                                url=server_queue[server_index[str(reaction.message.guild.id)]][1]).author, inline=True)
                            embed.add_field(name="Duration", value=time_converter(pytube.YouTube(
                                url=server_queue[server_index[str(reaction.message.guild.id)]][1]).length), inline=True)
                            embed.set_footer(text="Voice Channel Bitrate: {} kbps".format(
                                reaction.message.guild.voice_client.channel.bitrate/1000))
                            await reaction.message.edit(embed=embed)
        ...
        if reaction.emoji == "*️⃣":
            ...
            if str(user) != str(bot.user) and reaction.message.author == bot.user:
                ...
                if len(server_queue) <= 0:
                    ...
                    try:
                        embed = discord.Embed(description="**Song: **{a}\n**Index: **{b}\n**Views: **{c}\n**Description: **\n{d}".format(a=server_queue[server_index[str(reaction.message.guild.id)]][0], b=server_index[str(
                            reaction.message.guild.id)], c=pytube.YouTube(url=server_queue[server_index[str(reaction.message.guild.id)]][1]).views, d=pytube.YouTube(url=server_queue[server_index[str(reaction.message.guild.id)]][1]).description), color=color)
                        embed.set_author(name="Currently Playing", url=server_queue[server_index[str(
                            reaction.message.guild.id)]][1], icon_url=url_author_music)
                        embed.set_footer(text="Voice Channel Bitrate: {} kbps".format(
                            reaction.message.guild.voice_client.channel.bitrate/1000))
                        embed.set_thumbnail(url=pytube.YouTube(
                            url=server_queue[server_index[str(reaction.message.guild.id)]][1]).thumbnail_url)
                        ...
                        embed = discord.Embed(
                            description="Looks like you weren't playing anything before this so there is no current song. Play song from queue to set a current song", color=color)
                        embed.set_author(
                            name="Uh oh...", icon_url=url_author_music)
                        await reaction.message.edit(embed=embed)
                ...
                if members_in_vc.count(str(user)) > 0:
                    ...
                    try:
                        ...
                        if reaction.message.guild.voice_client.is_playing() != True:
                            embed = discord.Embed(description="**Song: **{}".format(server_queue[server_index[str(
                                reaction.message.guild.id)]][0]).replace(" - YouTube", " "), color=color)
                            embed.set_author(
            ...
            if str(user) != str(bot.user) and reaction.message.author == bot.user:
                ...
                if members_in_vc.count(str(user)) > 0:
                    ...
                    if reaction.message.guild.voice_client.is_playing() == False:
                        ...
                        embed.add_field(name="Uploader", value=pytube.YouTube(
                            url=random_song[1]).author, inline=True)
                        embed.add_field(name="Duration", value=time_converter(
                            pytube.YouTube(url=random_song[1]).length), inline=True)
                        embed.set_footer(
                            text=f"Voice Channel Bitrate: {reaction.message.guild.voice_client.channel.bitrate/1000} kbps")
                        await reaction.message.edit(embed=embed)
                        voice.play(discord.FFmpegPCMAudio(
...
async def spiderman_signal(ctx):
    ...
    calls = [f"{ctx.author.name} is calling you!", f"Your aid has been requested by {ctx.author.name}.",
             f"{ctx.author.name} has got something for ya.", f"{ctx.author.name} requires your assistance.", f"{ctx.author.name} has called."]
...
async def get_quips(ctx):
    ...
    try:
        embed = discord.Embed(title=random.choice(
            titles), description=random.choice(dialogue_list), color=color)
        embed.set_thumbnail(url=random.choice(url_thumbnails))
...
async def IMDb_movies(ctx, *, movie_name=None):
    ...
    if movie_name is not None:
        ...
        try:
            ...
            title = movie[0]['title']
            movie_summary = db.get_movie(movie[0].getID()).summary().replace("=", "").replace("Title", "**Title**").replace("Movie", "").replace("Genres", "**Genres**").replace("Director", "**Director**").replace(
                "Writer", "**Writer(s)**").replace("Cast", "**Cast**").replace("Country", "**Country**").replace("Language", "**Language**").replace("Rating", "**Rating**").replace("Plot", "**Plot**").replace("Runtime", "**Runtime**")
            movie_cover = movie[0]['full-size cover url']
            embed = discord.Embed(title="🎬 {} 🍿".format(
                title), description=movie_summary, color=color)
...
async def reddit_memes(ctx, *, topic):
    ...
    try:
        ...
        embed = discord.Embed(
            description="Looks like the subreddit is either banned or does not exist 🤔", color=color)
    ...
    try:
        ...
        try:
            ...
            embed = discord.Embed(description=wikipedia.summary(
                thing_to_search), color=color)
            embed.set_author(name=title.title, icon_url=url_wiki)
            embed.add_field(name='Search References', value=', '.join(
                [x for x in wikipedia.search(thing_to_search)][:5]), inline=False)
            embed.set_footer(text="Searched by: {}".format(
...
async def stop_program(ctx):
    ...
    msgs = [f"Bye {ctx.author.name}!", f"See ya {ctx.author.name}!",
...
async def conduct_poll(ctx, ems=None, title=None, *, description=None):
    ...
    if ems is not None and title is not None and description is not None:
        ...
        embed = discord.Embed(
            title="Polls", description="Command: `_polls emojis title description`", color=color)
...
async def snipe(ctx):
    ...
    try:
        ...
        if len(message) < 3:
            ...
            embed.set_footer(text=f"Sent by: {bot.get_user(int(message[0]))}", icon_url=bot.get_user(
                int(message[0])).avatar_url)
            await ctx.send(embed=embed)

        else:
            embed = discord.Embed(
                description="Embed deleted 👇🏻", color=color)
            embed.set_author(name=bot.get_user(
                int(message[0])), icon_url=bot.get_user(int(message[0])).avatar_url)
            await ctx.send(embed=embed)
...
async def user_pfp(ctx, member: discord.Member = None):
    ...
    embed.set_footer(text=random.choice(
        compliments), icon_url="https://i.pinimg.com/236x/9f/9c/11/9f9c11d4eaa3d99bc9a8ece092f5e979.jpg")
    await ctx.send(embed=embed)
...
async def server_information(ctx):
    ...
    embed.add_field(name="Role Count",
                    value=f"`{role_count}`", inline=True)
    embed.add_field(name="Region", value=f"`{region}`", inline=True)
    embed.add_field(name="Server ID", value=f"`{ID}`", inline=False)
    embed.add_field(name="Description",
                    value=f"```{description}```", inline=False)
    embed.set_footer(
        text=f"Created on {ctx.guild.created_at.__format__('%A, %B %d, %Y @ %H:%M:%S')}", icon_url=ctx.author.avatar_url)
...
async def encrypt_data(ctx, mode, *, message):
    ...
    try:
        ...
        if mode == "en":
            embed = discord.Embed(title="Message Encrpyted", description="```{}```".format(
                str(cipher.encrypt(res))).replace("b'", "").replace("'", ""), color=color)
            embed.set_thumbnail(url=url_en_dec)
            await ctx.channel.purge(limit=1)
            await ctx.send(embed=embed)

        if mode == "dec":
            embed = discord.Embed(title="Message Decrypted", description="```{}```".format(
                str(cipher.decrypt(res))).replace("b'", "").replace("'", ""), color=color)
            embed.set_thumbnail(url=url_en_dec)
            await ctx.channel.purge(limit=1)
            await ctx.send(embed=embed)
...
async def date_time_ist(ctx, timezone=None):
    ...
    if timezone is None:
        ...
        embed = discord.Embed(color=color)
        embed.add_field(name='Date', value="%s/%s/%s" %
...
async def get_calendar(ctx, year, month):
    ...
    try:
        embed = discord.Embed(title="Calendar", description="```{}```".format(
            calendar.month(int(year), int(month))), color=color)
        embed.set_thumbnail(url=url_dtc)
        await ctx.send(embed=embed)

    except IndexError:
        embed = discord.Embed(description="{}, this month doesn't exist 📆".format(
            ctx.author.name), color=color)
...
async def sql_shell(ctx, *, expression):
    ...
    try:
        ...
        conn.commit()
        embed = discord.Embed(title=str(expression),
                              description=str(output), color=color)
...
async def function_info(ctx, func):
    ...
    try:
        ...
        if "(" in [char for char in func] and ")" in [char for char in func]:
            ...
            function = eval(func)
            embed = discord.Embed(
                description=function.__doc__, color=color)
...
async def queue_song(ctx, *, name=None):
    ...
    if ctx.author.id not in [member.id for member in ctx.guild.voice_client.channel.members]:
        ...
        if name is not None:
            ...
            name_of_the_song = htm_code[starting:ending].replace(
                "&#39;", "'").replace("&amp;", "&")
            ...
            if url in check_list:
                ...
                embed = discord.Embed(description=f"{name_of_the_song}".replace(
                    " - YouTube", " "), color=color)
                embed.set_author(name="Song added", icon_url=url_author_music)
                await ctx.send(embed=embed)

        else:
            operation_view = "SELECT song_name, song_url FROM music_queue WHERE server={}".format(
                str(ctx.guild.id))
            cursor.execute(operation_view)
                ...
                try:
                    ...
                    if server_index[str(ctx.guild.id)] > 10:
                        ...
                        embed.set_thumbnail(
                            url=random.choice(url_thumbnail_music))
                        embed.set_footer(
                            text=f'Number Of Songs: {len(songs)}')
            ...
            try:
                ...
                if char.isdigit() == False:
                    ...
                    if ctx.voice_client.is_playing() != True:
                        embed = discord.Embed(
                            description="**Song: **{}".format(name_of_the_song).replace(" - YouTube", " "), color=color)
                        embed.set_author(
                            name="Now playing", url=url, icon_url=url_author_music)
                        embed.set_thumbnail(
                            url=pytube.YouTube(url=url).thumbnail_url)
                        embed.add_field(name="Uploader", value=pytube.YouTube(
                            url=url).author, inline=True)
                        embed.set_footer(text="Voice Channel Bitrate: {} kbps".format(
                            ctx.guild.voice_client.channel.bitrate/1000))
                        embed.add_field(name="Duration", value=time_converter(
                            pytube.YouTube(url=url).length), inline=True)
                        player = await ctx.send(embed=embed)
                        voice.play(discord.FFmpegPCMAudio(
                    ...
                    try:
                        ...
                        if ctx.voice_client.is_playing() != True:
                            embed = discord.Embed(description="**Song: **{a}\n**Queue Index: **{b}".format(
                                a=server_queue[int(char)][0], b=char).replace(" - YouTube", " "), color=color)
                            embed.set_author(
                                name="Now playing", icon_url=url_author_music)
                            embed.set_thumbnail(url=pytube.YouTube(
                                url=server_queue[int(char)][1]).thumbnail_url)
                            embed.set_footer(text="Voice Channel Bitrate: {} kbps".format(
                                ctx.guild.voice_client.channel.bitrate/1000))
                            embed.add_field(name="Uploader", value=pytube.YouTube(
                                url=server_queue[int(char)][1]).author, inline=True)
                            embed.add_field(name="Duration", value=time_converter(
                                pytube.YouTube(url=server_queue[int(char)][1]).length), inline=True)
                            voice.play(discord.FFmpegPCMAudio(
                                URL_queue, **FFMPEG_OPTS))
                            player = await ctx.send(embed=embed)
                ...
                embed = discord.Embed(description='I am not connected to a voice channel'.format(
                    ctx.author.name), color=color)
...
async def fetch_current_song(ctx):
    ...
    if len(server_queue) <= 0:
        ...
        try:
            embed = discord.Embed(description="**Song: **{a}\n**Index: **{b}\n**Views: **{c}\n**Description: **\n{d}".format(a=server_queue[server_index[str(ctx.guild.id)]][0], b=server_index[str(
                ctx.guild.id)], c=pytube.YouTube(url=server_queue[server_index[str(ctx.guild.id)]][1]).views, d=pytube.YouTube(url=server_queue[server_index[str(ctx.guild.id)]][1]).description), color=color)
            embed.set_thumbnail(url=pytube.YouTube(
                url=server_queue[server_index[str(ctx.guild.id)]][1]).thumbnail_url)
            embed.set_footer(text="Voice Channel Bitrate: {} kbps".format(
                ctx.guild.voice_client.channel.bitrate/1000))
...
async def previous_song(ctx):
    ...
    if ctx.author.id in [member.id for member in ctx.voice_client.channel.members]:
        ...
        try:
            ...
            if ctx.voice_client.is_playing() != True:
                embed = discord.Embed(description="**Song: **{}".format(
                    server_queue[server_index[str(ctx.guild.id)]][0]).replace(" - YouTube", " "), color=color)
                embed.set_author(name="Now playing",
                                 icon_url=url_author_music)
                embed.set_thumbnail(url=pytube.YouTube(
                    url=server_queue[server_index[str(ctx.guild.id)]][1]).thumbnail_url)
                embed.add_field(name="Uploader", value=pytube.YouTube(
                    url=server_queue[server_index[str(ctx.guild.id)]][1]).author, inline=True)
                embed.add_field(name="Duration", value=time_converter(pytube.YouTube(
                    url=server_queue[server_index[str(ctx.guild.id)]][1]).length), inline=True)
                embed.set_footer(text="Voice Channel Bitrate: {} kbps".format(
                    ctx.guild.voice_client.channel.bitrate/1000))
                player = await ctx.send(embed=embed)
                voice.play(discord.FFmpegPCMAudio(
...
async def remove_song(ctx, index):
    ...
    await ctx.send(embed=embed)
    operation_remove = "DELETE FROM music_queue WHERE song_url = '{a}' AND server='{b}'".format(
...
async def check_user_bdays_and_wish(ctx):
    ...
    for bday in bdays:  # bday[0]   bday[1]  bday[2]
        ...
        if datetime.datetime.today().month == bday[1] and datetime.datetime.today().day == bday[2]:
            ...
            wishes = [f"🎊 Happy Birthday {name} 🎊", f"🎉 Happy Birthday {name} 🎉",
                      f"✨ Happy Birthday {name} ✨", f"🎇 Happy Birthday {name} 🎇"]
            embed = discord.Embed(title=random.choice(
